...
![a path and a sphere](pathAndSphere.png)
...
We would like to measure how compactly the set of points $latex
...
## Does a path confined to a direction consistently move in that
   direction?

The answer here is no.  Consider a path which starts out at the origin
and the heads out along the x-axis.  Perhaps is gets a little ways out
and then takes to wandering back and forth.  As long as it never quite
gets back to the origin, the methods we have described will show this
path as highly directional.  Accordingly if you have determined that
your path has strong directionality using these methods, it is
appropriate to check for progression in that direction.  WRITE A
FUNCTION FOR THIS AND INCLUDE AN EXAMPLE HERE.

## Stability

DISCUSS BEHAVIOUR IN THE REGION NEAR THE ORIGIN
